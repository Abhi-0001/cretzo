...
// Forgot-password (admin/seller/delivery_boy) used to live here: a Firebase
// phone-auth + reCAPTCHA flow that called auth/verify_user (a signup
// duplicate-check being repurposed backwards for this) then
// firebase.auth().signInWithPhoneNumber(), hardcoded to always submit the
// final reset to admin/home/reset-password regardless of which panel the
// form was actually on - so a seller/delivery-boy password reset always hit
// the admin-only endpoint. It also depended on a #recaptcha-container-2 div
// that no longer exists in any of the three views. Replaced by a proper,
// backend-verified OTP flow with its own small submit handlers written
// directly into each panel's forgot-password view (seller/login.php,
// admin/pages/forms/forgot-password.php, delivery_boy/pages/forms/forgot-password.php),
// each posting to that panel's own send_reset_otp/reset_password endpoints.
