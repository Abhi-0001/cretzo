        ...
        if (!$this->form_validation->run()) {
            ...
            if ($this->Order_model->add_bank_transfer_proof($data)) {
                ...
                if (!empty($fcm_ids)) {
                    ...
                    $proof_title = (!empty($custom_notification) && !empty($custom_notification[0]['title']))
                        ? render_notification_text($custom_notification[0]['title'], ['order_id' => $order_id, 'application_name' => $app_name])
                        : "You have new order proof";
                    $fcmMsg = array(
                        'title' => $proof_title,
                        'body' => $customer_msg,
    ...
    public function phonepe_app()
    {
        ...
        */
        ...
        if (isset($_POST['type']) && $_POST['type'] == 'wallet') {
            ...
            $res = $this->phonepe->phonepe_payment($data);
            $this->response['error'] = false;
            $this->response['data'] = $res;
            $this->response['message'] = $res['message'];
            $this->response['mobileNumber'] = $data['mobileNumber'];
            $this->response['merchantUserId'] = $data['merchantUserId'];
            $this->response['redirectMode'] = $data['redirectMode'];
            $this->response['paymentInstrument'] = $data['paymentInstrument'];
            $this->response['payment_url'] = $res['data']['instrumentResponse']['intentUrl'];
            print_r(json_encode($this->response));
            return;
        } else {
