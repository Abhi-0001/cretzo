...
class Seller_privacy_policy extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        if (!has_permissions('read', 'seller_privacy_policy')) {
    ...
    public function update_privacy_policy_settings()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            if (print_msg(!has_permissions('update', 'seller_privacy_policy'), PERMISSION_ERROR_MSG, 'seller_privacy_policy')) {
                return false;
            }
            ...
            if (!$this->form_validation->run()) {
                ...
                $updated = $this->Setting_model->update_seller_privacy_policy($_POST);
                $updated = $this->Setting_model->update_seller_terms_n_condtions($_POST) && $updated;
