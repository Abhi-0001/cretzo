    ...
    public function get_groups()
    {
        ...
        */
        ...
        if (!$this->form_validation->run()) {
            ...
            /*
             * This endpoint called Chat_model::get_groups() and ::get_group_members(), NEITHER of
             * which exists - both are commented out in the model along with the rest of the
             * group-chat feature, and the `chat_groups` / `chat_group_members` tables are not in
             * the schema. Any call therefore died with an uncaught
             *   PHP Error - Call to undefined method Chat_model::get_groups()
             * It was latent only because the whole seller chat API answered 403 on every request
             * (missing from csrf_exclude_uris); with that fixed it became reachable, so it has to
             * answer honestly instead of crashing. Group chat is not part of this product.
             */
            $this->response['error'] = true;
